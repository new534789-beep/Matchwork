...
/** Modèle par défaut : en-tête offre + titre du document + corps. */
...
export type DossierPourPdf = {
  opportunite: { intitule: string; organisme: string };
  docsGeneres: { type: string; contenu: string }[];
};

/**
 * Génère le PDF complet d'un dossier (CV en page(s) sidebar + chaque autre document
 * en page(s) classiques) — logique PARTAGÉE entre le téléchargement (/pdf) et
 * l'envoi par e-mail (/email), pour ne jamais envoyer au recruteur un document
 * différent de celui que le candidat a vu/téléchargé.
 */
export async function genererDossierPdf(dossier: DossierPourPdf, profil: InfosProfil): Promise<Uint8Array> {
  ...
  for (const doc of dossier.docsGeneres) {
    if (doc.type === "cv") {
      dessinerCvSidebar(pdf, doc, profil, fonts, c);
    } else {

...
export default async function FilStages() {
  ...
  const [fil, profil] = await Promise.all([
    obtenirFilOpportunites(session.user.id, "STAGE"),
  ...
  return (
    <>
      <EnteteApp titre="Stages" retour="/opportunites" />
      <main className="flex-1 px-4 py-6 max-w-xl mx-auto w-full">
        <div className="mb-6">
          <h2 className="text-lg font-bold" style={{ color: "var(--text)" }}>Fil des stages</h2>

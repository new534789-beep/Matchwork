...
export function ChatProjetGeneral() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch("/api/profil/brief-projet", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ message: "Bonjour, je veux accéder aux appels à projets.", historique: [] }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.reponse) {
  ...
  async function envoyer() {
    ...
    try {
      const res = await fetch("/api/profil/brief-projet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      >
        ...
        {termine && (
          <div style={{ textAlign: "center", marginTop: 16 }}>
            <button
              onClick={() => router.push("/opportunites/appels-projets")}
              style={{
                background: V,
                color: "#fff",
                border: "none",
